...
# Add KAIR path for DnCNN and FFDNet
sys.path.append(r"D:\02_Projects\ML\KAIR")
try:
    from models.network_dncnn import DnCNN as net_dncnn
    from models.network_ffdnet import FFDNet as net_ffdnet
except ImportError:
    print("Error: Could not load KAIR models. Check path.")
...
def run_DnCNN(img_noisy_t, device, model_pool=r"D:\02_Projects\ML\KAIR\model_zoo", model_name='dncnn_25'):
    ...
    model = net_dncnn(in_nc=n_channels, out_nc=n_channels, nc=64, nb=nb, act_mode='R')
...
# 4. FFDNet Runner
# ==========================================
def run_FFDNet(img_noisy_t, sigma, device, model_pool=r"D:\02_Projects\ML\KAIR\model_zoo", model_name='ffdnet_gray'):
    n_channels = 1
    nc = 64
    nb = 15
    model_path = os.path.join(model_pool, model_name+'.pth')
    
    model = net_ffdnet(in_nc=n_channels, out_nc=n_channels, nc=nc, nb=nb, act_mode='R')
    model.load_state_dict(torch.load(model_path, map_location=device), strict=True)
    model.eval()
    for k, v in model.named_parameters():
        v.requires_grad = False
    ...
    sigma_tensor = torch.full((1, 1, 1, 1), sigma).type_as(img_noisy_t).to(device)
    with torch.no_grad():
        img_E = model(img_noisy_t, sigma_tensor)
...
def main():
    ...
    sigma = 25.0 / 255.0  # evaluate on noise level 25
    ...
    print("Running DnCNN...")
    t0 = time.time()
    res_dncnn = run_DnCNN(img_noisy_t, device, model_name='dncnn_25')
    results['DnCNN'] = {'img': res_dncnn.squeeze().cpu().numpy(), 'time': time.time() - t0}
    
    # 5. FFDNet
    print("Running FFDNet...")
    t0 = time.time()
    res_ffdnet = run_FFDNet(img_noisy_t, sigma, device, model_name='ffdnet_gray')
    results['FFDNet'] = {'img': res_ffdnet.squeeze().cpu().numpy(), 'time': time.time() - t0}
    ...
    # Ensure ordered tracking
    method_order = ['BM3D', 'ISTA', 'FISTA', 'ADMM', 'DnCNN', 'FFDNet']
    ...
    # Save individual images for paper inclusion
    ...
    # Detailed Visualization for Paper (Grid 2x4 = 8 images)
    ...
    fig_patch, axes_p = plt.subplots(1, 5, figsize=(25, 5))
    patch_names = ['Ground Truth', 'BM3D', 'ADMM', 'DnCNN', 'FFDNet']
    ...
    for name in patch_names:
        ...
        axes_p[valid_patch_idx].imshow(patch, cmap='gray')
        axes_p[valid_patch_idx].set_title(f"Zoomed: {name}", fontsize=14)
        axes_p[valid_patch_idx].axis('off')
        valid_patch_idx += 1
        
    # Hide any unused subplots
    for i in range(valid_patch_idx, 5):
        axes_p[i].axis('off')
