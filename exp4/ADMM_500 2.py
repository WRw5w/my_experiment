...
def run_AMDD(y, lam=0.08, rho=10.0, num_iters=300, original_img=None, logger=None):
    x = y.clone()
    ...
    # 根据 Hessian 最大特征值动态计算正确的学习率，防止发散
    # H = I + rho * (Dx^T Dx + Dy^T Dy), eigenvalue_max <= 1 + 8 * rho
    lr = 1.0 / (1.0 + 8.0 * rho)
    ...
    for i in range(num_iters):
        # ====================
        # 1. 更新 x (保真项 + 二次惩罚项)
        # argmin_x 0.5||x - y||^2 + (rho/2)||Dx - z + u||^2
        # ====================
        # 增加内循环以更精确地求解子问题
        for _ in range(5):
            dx = diff_x(x)
        ...
        # argmin_z lam ||z||_1 + (rho/2)||Dx - z + u||^2
        # ====================
        dx = diff_x(x)
        dy = diff_y(x)
        
        # 联合计算梯度范数 (Isotropic TV)
        vx = dx + u_x
        vy = dy + u_y
...
def ista_tv(noisy, lambd, num_iter=50, original_img=None, logger=None):
    ...
    for i in range(num_iter):
        div_tv = calc_div_tv(x)
        grad = (x - noisy) - lambd * div_tv
        x = x - step_size * grad
...
def fista_tv(noisy, lambd, num_iter=50, original_img=None, logger=None):
    ...
    for i in range(num_iter):
        ...
        # Gradient is computed at Lookahead position 'y'
        div_tv = calc_div_tv(y)
        grad = (y - noisy) - lambd * div_tv
        x = y - step_size * grad
...
def compare_and_visualize_demo():
    ...
    # === Hyperparameters ===
    lam_amdd = 0.1 # 严格对齐 ISTA 的 lambda
    rho_amdd = 5.0
    lam_tv = 0.1
    iters = 500 
    ...
    # 1. AMDD
    print(f"\n[1/4] Running AMDD (Ours), iters={iters}...")
    loggers["AMDD"] = PerfLogger("AMDD", image_np, log_iter_freq, log_time_freq, thresholds)
    res_amdd, psnr_amdd_h, ssim_amdd_h = run_AMDD(
        img_noisy_t, lam=lam_amdd, rho=rho_amdd, num_iters=iters, original_img=img_clean_t, logger=loggers["AMDD"]
    )
