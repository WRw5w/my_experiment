"""
U-Net Denoising — Testing & Evaluation Script
===============================================
Test the trained U-Net model on DIV2K validation set across multiple
noise levels (σ = 15, 25, 35, 50). Compare with BM3D and FFDNet.
Generate PSNR/SSIM tables and visual comparison figures.

Usage:
    python test.py --mode gray
    python test.py --mode color
    python test.py --mode gray --sigmas 15 25 35 50
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Test U-Net denoiser")
...
def denoise_unet(model, noisy_np, device, mode):
    """Denoise a single image using U-Net."""
    if noisy_np.ndim == 2:
...
def denoise_ffdnet(noisy_np, sigma, device):
    """Denoise using FFDNet (pretrained from KAIR)."""
    is_gray = noisy_np.ndim == 2
    n_ch = 1 if is_gray else 3
    model_name = "ffdnet_gray" if is_gray else "ffdnet_color"
    model_path = os.path.join(r"D:\02_Projects\ML\KAIR\model_zoo", f"{model_name}.pth")

    if not os.path.exists(model_path):
        print(f"  FFDNet model not found at {model_path}, skipping.")
        return None

    model = net_ffdnet(in_nc=n_ch, out_nc=n_ch, nc=64, nb=15, act_mode='R')
    model.load_state_dict(torch.load(model_path, map_location=device), strict=True)
    model.eval().to(device)

    if is_gray:
        t = torch.from_numpy(noisy_np).unsqueeze(0).unsqueeze(0).float().to(device)
    else:
        t = torch.from_numpy(noisy_np.transpose(2, 0, 1)).unsqueeze(0).float().to(device)

    sigma_t = torch.full((1, 1, 1, 1), sigma / 255.0).float().to(device)
    with torch.no_grad():
        out = model(t, sigma_t).clamp(0, 1)
    return tensor_to_numpy(out)
...
def main():
    ...
    if args.model_path:
        ...
        ckpt_path = os.path.join(script_dir, "checkpoints",
                                 f"unet_{args.mode}_sigma25_best.pth")
    ...
    for sigma in args.sigmas:
        ...
        for idx, (clean, name) in enumerate(zip(images, names)):
            ...
            # --- U-Net ---
            t0 = time.time()
            denoised_unet = denoise_unet(model, noisy, device, args.mode)
            all_results[sigma]["U-Net"]["time"] += time.time() - t0
            all_results[sigma]["U-Net"]["psnr"].append(compute_psnr(clean, denoised_unet))
            all_results[sigma]["U-Net"]["ssim"].append(compute_ssim(clean, denoised_unet))
            ...
            if HAS_FFDNET:
                t0 = time.time()
                denoised_ffdnet = denoise_ffdnet(noisy, sigma, device)
                if denoised_ffdnet is not None:
                    all_results[sigma]["FFDNet"]["time"] += time.time() - t0
                    all_results[sigma]["FFDNet"]["psnr"].append(
                        compute_psnr(clean, denoised_ffdnet))
                    all_results[sigma]["FFDNet"]["ssim"].append(
                        compute_ssim(clean, denoised_ffdnet))
            ...
            if idx < 2:
                ...
                if HAS_FFDNET and denoised_ffdnet is not None:
                    visual_samples[key]["FFDNet"] = denoised_ffdnet
    ...
    for (sigma, name), sample in visual_samples.items():
        imgs = [sample["clean"], sample["noisy"], sample["U-Net"]]
        noisy_psnr = compute_psnr(sample["clean"], sample["noisy"])
        unet_psnr = compute_psnr(sample["clean"], sample["U-Net"])
        titles = [
            "Ground Truth",
        ...
        if "FFDNet" in sample:
            ffdnet_psnr = compute_psnr(sample["clean"], sample["FFDNet"])
            imgs.append(sample["FFDNet"])
            titles.append(f"FFDNet ({ffdnet_psnr:.2f}dB)")
        ...
        save_comparison_figure(imgs, titles, save_path,
                               suptitle=f"Denoising Comparison — σ={sigma}, {name}",
                               cmap=cmap)

    # ---------- PSNR vs Sigma plot ----------
    print("Generating PSNR vs σ plot ...")
    fig, ax = plt.subplots(figsize=(8, 5))
    markers = {"U-Net": "o-", "BM3D": "s--", "FFDNet": "^-."}
