...
# 噪声添加函数
def add_noisy(x, sigma=25, salt_pepper_prob=0.02):
    """x: [B, C, H, W]"""
    noise = torch.normal(mean=0, std=sigma/255.0, size=x.shape).to(x.device)
    x_noisy = x + noise
    if salt_pepper_prob > 0:
        prob = torch.rand(x.shape).to(x.device)
        x_noisy[prob < salt_pepper_prob / 2] = 0
        x_noisy[prob > 1 - salt_pepper_prob / 2] = 1
    return torch.clamp(x_noisy, 0, 1)

# ==========================================
# 2. PyTorch GPU版 BM3D (核心修改)
# ==========================================
class BM3D_GPU_Solver:
...
def run_compare(image_path, sigma=25):
    ...
    if imag.ndim == 3: imag = color.rgb2gray(imag)

    # 转 Tensor: [1, 1, H, W]
    x = torch.tensor(img_as_float(imag), dtype=torch.float32).to(device)
    x = x.unsqueeze(0).unsqueeze(0)

    # 加噪
    x_noisy = add_noisy(x, sigma)

    # BM3D 去噪
    solver = BM3D_GPU_Solver(sigma)

    torch.cuda.synchronize()
    t0 = time.time()
    x_clean = solver.denoise(x_noisy)
    torch.cuda.synchronize()
    t_cost = time.time() - t0

    # 计算指标
    psnr_val = psnr_metric(x.cpu().numpy().squeeze(),
                           x_clean.cpu().numpy().squeeze(),
                           data_range=1.0)

    print(f"File: {os.path.basename(image_path)} | PSNR: {psnr_val:.2f} | Time: {t_cost:.4f}s")
    return psnr_val, t_cost

if __name__ == "__main__":
    if not os.path.exists(INPUT_DIR):
        print("Input dir not found.")
    else:
        # 只跑前一张测试，因为 BM3D 还是比较重的
        files = [f for f in os.listdir(INPUT_DIR) if f.endswith(('.png', '.jpg'))]
        for f in files[:16]:
            run_compare(os.path.join(INPUT_DIR, f))